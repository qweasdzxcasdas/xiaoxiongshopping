        ...
        <div class="bread">
          ...
          <ul class="fl sui-tag">
            <li class="with-x" v-if="this.searchParams.categoryName">
              {{ this.searchParams.categoryName }}<i @click="delSearch">×</i>
            </li>
            <li class="with-x" v-if="this.searchParams.keyword">
              {{ this.searchParams.keyword }}<i @click="delKeyword">×</i>
            </li>
            <!-- 品牌信息面包屑 -->
        ...
        <div class="details clearfix">
          <div class="sui-navbar">
            <div class="navbar-inner filter">
              <ul class="sui-nav">
                <li :class="{ active: isOne }" @click="changBtn(1)">
                  <a
                    >综合<span
                      v-show="isOne"
                      class="iconfont"
                ...
                <li :class="{ active: isTwo }" @click="changBtn(2)">
                  <a
                    >价格<span
                      v-show="isTwo"
                      class="iconfont"
          ...
          <pagination
            @pageemit="pageemit"
            :pageNow="searchParams.pageNo"
            :pageInfo="searchParams.pageSize"
            :total="total"
            :cont="5"
          />
        </div>
...
export default {
  ...
  methods: {
    ...
    delKeyword() {
      (this.searchParams.keyword = ""), this.goSearch();
      this.$bus.$emit("clear");
      // if(this.$route.query){
    ...
    changBtn(flag) {
      ...
      let oldtwo = this.searchParams.order.split(":")[1];
      let newFlag = "";
      if (flag == oldflag) {
        newFlag = `${oldflag}:${oldtwo == "desc" ? "asc" : "desc"}`;
        console.log("1");
      } else {
        newFlag = `${flag}:${"desc"}`;
      }
      this.searchParams.order = newFlag;
      this.goSearch();
    },
    pageemit(pageNow) {
