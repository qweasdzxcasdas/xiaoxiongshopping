...
export default {
  ...
  methods: {
    async getmsginfo() {
      let result = await this.$API.reqGetmsg(this.orderId);
      if (result.code == 200) {
    ...
    async open() {
      ...
      this.$alert(`<img src=${img} />`, "微信支付", {
        ...
        beforeClose:((type,instance,done)=>{
          //type:区分取消|确定按钮 instance：组件实例方法 done：关闭窗口
          if(type=='cancel'){
            alert('请联系牛子豪')
      ...
      if (!this.timer) {
        this.timer = setInterval(async () => {
          let result = await this.$API.reqpay(this.orderId);
          if (result.code == 200) {
            clearInterval(this.timer)
            this.timer=null
