          ...
          <div class="goodsDetail">
            ...
            <div class="support">
              ...
              <div class="supportArea">
                ...
                <div class="fixWidth">湖北省 武汉市 江岸区</div>
              </div>
            </div>
          </div>
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(spuSale,index) in spuSaleAttrList" :key="spuSale.id">
                ...
                <dd @click="isChecked(spu,spuSale.spuSaleAttrValueList)" changepirce="0" :class="{active:spu.isChecked==1}" v-for="(spu,index) in spuSale.spuSaleAttrValueList" :key="spu.id">{{spu.saleAttrValueName}}</dd>
                
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model="skyNum" @change="inputsky">
                <a href="javascript:" class="plus" @click="skyNum++">+</a>
                <a href="javascript:" class="mins" @click="sky">-</a>
              </div>
              <div class="add">
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      inputsky(event){
        let e = event.target.value *1
        if(isNaN(e)||e<1){
          this.skyNum=1
        }else{
          this.skyNum=parseInt(e)
        }
      },
      //添加到购物车
      async addshopcar(){
        ...
        try {
          await this.$store.dispatch('addorupdataShopcat',{skuId:this.$route.params.skuId,skuNum:this.skyNum})
          //路由跳转.会话存储
